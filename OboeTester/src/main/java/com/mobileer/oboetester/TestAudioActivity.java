...
//    public ArrayList<String> statusValue = new ArrayList();
...
//Mohsin was here
    ...
    public void setConfigViewsEnabled(boolean b) {
        try {
            for (StreamContext streamContext : mStreamContexts) {
                if (streamContext.configurationView != null) {
    ...
    public void startAudio(inComFailedCallBack inComFailedCallBack) throws IOException {
        int result = startNative();
        if (result < 0) {
            showErrorToast("Start failed with " + result);
            inComFailedCallBack.ignoreInCom();
